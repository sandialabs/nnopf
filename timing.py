...
model.load_state_dict(
    torch.load("trained_models/ffnn_pglib_opf_case30_ieee_best.pt")
...
for f in files:
    ...
    md_nominal, nominal_acopf, status = dcopf.solve_dcopf(
        md, "gurobi", solver_tee=True, return_model=True, return_results=True
    )
    ...
    start_time = time.time()
    md_, n, s = solve_acopf(
        md, "ipopt", solver_tee=True, return_model=True, return_results=True
    )
    end_time = time.time()
