def add_generator_to_m_file(input_file_path, output_file_path, new_gen, new_gencost):
    ...
    for i, line in enumerate(lines):
        if line.startswith("function mpc ="):
            lines[i] = f"function mpc = {output_file_path[11:-2]}\n"
...
for bus in [1, 2, 3, 6, 8]:
    ...
    new_gencost = "2\t 0.0\t 0.0\t 3\t 0.000000\t 12\t 0.000000"

    output_file_path = f"CASE14_bus/pglib_opf_case14_ieee_modified_bus_{bus}.m"

    add_generator_to_m_file(
        input_file_path, output_file_path, new_generator, new_gencost
    )
